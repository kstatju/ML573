...
def classify(indx, total_class, vocab_len, dt, prob, condi_prob):
    ...
    a3 = np.transpose(a1*np.log(np.transpose(condi_prob)));
    a3[np.isnan(a3)]=0;
    res = np.argmax(np.transpose(np.log(prob)) + sum(a3))+1;
    del a1, a2, a3;
    return int(res);
...
for j in range(0,dim_omega):
    ...
    cond_prob[:,j] = cont_vec[:,j]/total_count;
